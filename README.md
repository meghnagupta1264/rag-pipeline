...
Change chunk size — delete `.chroma/` and re-index with `chunk_size=200`. Ask the same questions. Smaller chunks are more precise but lose surrounding context. Larger chunks keep context but retrieve noisier results.

```bash
rm -rf .chroma
python main.py
```
Multi-document retrieval — add two or three different PDFs to `docs/`. Ask questions that span both. Watch the retriever pull chunks from multiple sources and the LLM synthesize across them.

Top-k tuning — change `top_k=5` to `top_k=2` for faster, more focused answers. Change to `top_k=10` for richer context. Notice where each setting breaks down.
